...
use member::{
    msg::{
        IsMemberResponse, MemberCountResponse, MembersResponse, MembershipSupplyResponse,
        MembershipsResponse, QueryIsMemberMsg, QueryMemberCountMsg, QueryMembersMsg,
        QueryMembershipSupplyMsg, QueryMembershipsMsg,
    },
...
pub fn query_is_member(deps: Deps, data: QueryIsMemberMsg) -> StdResult<IsMemberResponse> {
    let user_id = data.user_id.u64();
    let membership_issuer_user_id = data.membership_issuer_user_id.u64();

    let amount = ALL_MEMBERSHIPS_MEMBERS
        .may_load(deps.storage, (membership_issuer_user_id, user_id))?
        .unwrap_or(Uint128::zero());

    Ok(IsMemberResponse {
        is_member: amount != Uint128::zero(),
        amount,
    })
}
